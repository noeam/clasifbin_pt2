...
datos <- read.csv("outputs/completo_seleccionadas.csv", header = TRUE)
...
ggplot(data = datos, aes(x = ejer0_10, fill=obesidad)) +
...
ggplot(data = datos, aes(x = obesidad, fill=ejer_act)) +
  geom_bar(stat = "count", colour = "white") +
...
ggplot(data = datos, aes(x = obesidad, fill=ejer_1)) +
  geom_bar(stat = "count", colour = "white") +
  ggtitle("Participantes por sexo") + 
  ylab("Cantidad") +
  xlab("Clase Obesidad o No Obesidad") + 
...
ejer_act <- datos$ejer_act
ejer_1 <- datos$ejer_1
BarChart(x = obesidad, by = ejer0_10, stack100 = TRUE)
BarChart(x = obesidad, by = ejer_act, stack100 = TRUE)
BarChart(x = ejer_act, by = obesidad, stack100 = TRUE)
...
nrow(datos[datos$obesidad == 1 & datos$ejer_10 == "M", ])
